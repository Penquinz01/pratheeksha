...
export const Contact: React.FC = () => {
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div className="lg:col-span-5 space-y-10">
              <div className="space-y-6">
                <h2 className="font-heading text-3xl font-bold text-brand-forest leading-tight">
                  Contact Information
                </h2>
                <p className="font-body text-sm sm:text-base text-brand-forest/75 leading-relaxed">
              ...
              <div className="space-y-6">
                {contactOptions.map((opt, idx) => (
                  <div key={idx} className="flex items-start space-x-4">
                    <div className="w-12 h-12 rounded-2xl bg-white text-brand-forest shadow-sm border border-brand-forest/5 flex items-center justify-center shrink-0">
                      {opt.icon}
                    </div>
                    <div>
                      ...
                      <div className="font-body text-xs sm:text-sm text-brand-forest/75 mt-1 space-y-0.5">
                        {opt.details.map((detail, dIdx) => (
                          <div key={dIdx}>
                            {opt.href && dIdx === 0 ? (
                              <a href={opt.href} className="hover:text-brand-emerald transition-colors font-medium">
                                {detail}
                              </a>
                            ) : (
                              <span>{detail}</span>
              ...
              {/* Social Channels */}
              <div className="space-y-3 pt-6 border-t border-brand-forest/5">
                ...
                <div className="flex space-x-3">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer" 
                    className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-brand-forest hover:bg-brand-emerald hover:text-white transition-colors duration-300 shadow-sm border border-brand-forest/5"
                    aria-label="Facebook"
                  >
            ...
            {/* Simple Contact Form */}
            <div className="lg:col-span-7 bg-white p-8 sm:p-10 rounded-3xl border border-brand-forest/5 shadow-md">
              ...
              {isSubmitted ? (
                <div className="bg-brand-forest/5 border border-brand-emerald/20 p-8 rounded-2xl text-center space-y-4">
                  <CheckCircle2 className="h-12 w-12 text-brand-emerald mx-auto" />
                  <h4 className="font-heading text-xl font-bold text-brand-forest">Message Dispatched</h4>
                  <p className="text-xs sm:text-sm text-brand-forest/75 font-body">Thank you for contacting Pratheeksha. We have logged your query and will reply shortly.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4 font-body">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider font-semibold text-brand-forest/80 mb-1">Your Name</label>
                      <input
                        type="text"
                        required
                        className="w-full bg-brand-beige/50 border border-brand-forest/10 focus:border-brand-emerald rounded-lg px-3 py-2.5 text-xs text-brand-forest outline-none"
                        value={formData.name}
                        onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider font-semibold text-brand-forest/80 mb-1">Email Address</label>
                      <input
                        type="email"
                        required
                        className="w-full bg-brand-beige/50 border border-brand-forest/10 focus:border-brand-emerald rounded-lg px-3 py-2.5 text-xs text-brand-forest outline-none"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider font-semibold text-brand-forest/80 mb-1">Phone Number</label>
                      <input
                        type="tel"
                        required
                        className="w-full bg-brand-beige/50 border border-brand-forest/10 focus:border-brand-emerald rounded-lg px-3 py-2.5 text-xs text-brand-forest outline-none"
                    ...
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider font-semibold text-brand-forest/80 mb-1">Subject</label>
                      <input
                        type="text"
                        required
                        className="w-full bg-brand-beige/50 border border-brand-forest/10 focus:border-brand-emerald rounded-lg px-3 py-2.5 text-xs text-brand-forest outline-none"
                  ...
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-brand-forest/80 mb-1">Your Message</label>
                    <textarea
                      rows={4}
                      required
                      className="w-full bg-brand-beige/50 border border-brand-forest/10 focus:border-brand-emerald rounded-lg px-3 py-2.5 text-xs text-brand-forest outline-none resize-none"
      ...
      <section className="bg-brand-beige border-t border-brand-forest/5">
